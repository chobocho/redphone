...
| `--scan` | `false` | 서브넷 전체에 주기적 유니캐스트 HELLO 스캔(막히는 망 주의) |

## 친구 IP 직접 추가 (브로드캐스트가 막힌 망)

방화벽·정책으로 UDP 브로드캐스트가 막히면 자동 발견이 안 될 수 있다. 이때는
사이드바 **친구 IP**에 상대 IP를 직접 등록한다(전체 IP 스캔이 막히는 환경 대비).

- **영속화** — 등록한 IP는 실행 파일과 같은 폴더의 `peers.json`에 원자적으로
  저장돼 재시작해도 유지된다. 목록에서 ✎(변경)·✕(삭제)로 관리한다.
- **단방향 등록으로 양방향 발견** — 한쪽만 상대 IP를 등록해도 서로를 발견한다.
  유니캐스트 HELLO를 받은 쪽이 1회 응답(`reply=true`)을 되돌리며, 응답의 응답은
  없어 발견 루프가 무한히 돌지 않는다.
- **전체 스캔(옵션)** — 🔍 **전체 스캔** 버튼(또는 `--scan`)은 서브넷의 모든
  호스트에 HELLO를 뿌려 검색한다. `/16` 이상으로 넓으면 본인 `/24`로 좁혀 폭주를
  막는다. 일부 망에서 차단/소음이 될 수 있어 **기본은 꺼져 있다.**

| 엔드포인트 | 설명 |
|---|---|
| `GET /api/targets` | 등록된 친구 IP 목록 |
| `POST /api/targets {ip}` | 추가(잘못된 IP는 400) |
| `PUT /api/targets {old,new}` | 변경(새 IP 유효할 때만 옛 IP 제거) |
| `DELETE /api/targets/{ip}` | 삭제 |
| `POST /api/scan` | 서브넷 전체 1회 스캔(`{sent}` 반환) |
...
- `internal/discovery` — UDP HELLO/BYE 인코딩·송수신 + 유니캐스트 친구 IP·전체 스캔
- `internal/peer` — Peer 레지스트리(upsert/expire/snapshot)
- `internal/peerbook` — 친구 IP 목록의 `peers.json` 영속화(원자적 저장)
...
4. **친구 IP 관리**: 사이드바 **친구 IP**에 `1.2.3.4` 입력→추가. 잘못된 값은
   거부 토스트. ✎로 변경, ✕로 삭제. 실행 파일 폴더에 `peers.json`이 생기고
   재시작 후에도 목록이 유지되는지 확인. 🔍 **전체 스캔** → "N개 호스트에 전송" 토스트.
5. **종료**: `exit` 입력 또는 ⏻ 종료 버튼 → 프로세스가 graceful 종료.
...
브로드캐스트가 막힌(또는 서브넷이 다른) 두 PC라면, **한쪽에서만** 상대 IP를
**친구 IP**에 등록한다 → 잠시 뒤 양쪽 피어 목록에 서로가 나타나면 유니캐스트
발견·reply 응답이 정상 동작하는 것이다.
